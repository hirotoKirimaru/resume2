...
- **エンタープライズ認証基盤（23 PR / 2021〜）**: SSO/SAML 設定の管理画面化、大手通信キャリア向け 2 要素認証（TOTP)、Google SSO(OAuth)からのカレンダー自動連携。IP アドレス制限などの個別要件対応
- **マルチテナント統合**: 複数ワークスペースを 1 つに統合する要件に対し、既存データ（1on1 記録・感想・ユーザー）を失わず・重複させず移行・集約。10 ワークスペースをデータロスト 0・無停止で統合完了
- **集計グループ機能（55 PR / 3 年）**: 組織構造に沿った集計の CSV 一括設定、ペア単位集計、組織変更追従、削除データの整合性担保まで継続オーナーシップ
...
- カレンダー連携（Google/Outlook, 29 PR）、満足度・サーベイ集計、管理画面・CSV 入出力など
...
- **基盤の大規模刷新（2026）**: Poetry→uv 移行＋Docker マルチステージビルド。全依存ライブラリの最新化（Python 3.12→3.13 含む）、ARM64 (Graviton)イメージ対応、Docker イメージ圧縮の zstd 化
- **可観測性刷新**: Sentry → Datadog APM への監視統合、業務エラーの検知対象切り分け
...
クラウドインフラを Terraform で管理（11 PR）。ECS Fargate のスペック最適化、Centrifugo 基盤、Lambda 管理、Datadog/Sentry 連携。アプリ開発と兼務する形で"アプリも書ける DevOps"として関与。
...
新規 SaaS プロダクトをゼロベースから本番リリースまで構築（110 PR）。面談フロー・予約フロー・外部連携（カレンダー/データ連携）・メール配信・タイムゾーン統一などの機能をゼロから実装。Sentry / CI / リリースフロー / lint といった"回り続ける運用基盤"も整備。
...
全期間で**673 件**のコードレビューを、全社リポジトリ横断・多言語（Ruby/Python/TypeScript/HCL）で担当。ピーク年は 303 件。チームの変更品質ゲートとして機能。
-->
