...
採用。
開発、PdM、QA、デザイナーを1つのチームにまとめたクロスファンクショナルチームの導入。
生成AIの導入により運用面でDevinを使用し仕様や不具合問い合わせ効率の向上, 開発面でCodex, Claude Codeを導入。
...
- **基盤の大規模刷新（2026）**: Poetry→uv 移行, Docker マルチステージビルドによりECRイメージを1100MBから250MBまで縮小。全依存ライブラリの最新化（Python 3.12→3.13 含む）、ARM64 (Graviton)イメージ対応、Docker イメージ圧縮の zstd 化
- **可観測性刷新**: Sentryの導入、および運用文化の醸成。業務エラーの検知対象切り分け
...
CodeRabbitというレビューAIを導入することにより、レビュー負荷の軽減。
