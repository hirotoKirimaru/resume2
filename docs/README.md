...
運用面では Devin により仕様・不具合問い合わせ対応を効率化し、開発の中断を削減しました。開発面では Codex, Claude Code を導入し、単なるツール導入に留まらず開発プロセス自体を再設計しています。
    ...
    - Claude Code のプラグイン共有や、ADR・Design Doc・PRD・openspec による事前のドキュメント作成を仕組み化。考慮不足を減らし、生成 AI の生成物のブレを抑制
    - コードレビューのプロセスを再設計。従来は人間がコードを直接レビューしていましたが、検証環境へのデプロイ後に Playwright CLI や AgentBrowser でテストケースの洗い出しと実施を行い、AWS CLI や Datadog で監視しながら問題がなくなるまで AI にループさせ、人間は最後に不足観点がないかをレビューする体制へ移行
