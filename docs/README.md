...
　私を一言で表すなら「業務完遂エンジニア」です。業務(ドメイン)を深く理解した上で、「これは難しい」と言われる要求でも、なんとしてでも形にして完遂することを信条としています。官公庁の法令業務から、法人向けの販売・出荷・審査といった商流まで、複数の業務領域で要件定義から運用までを一気通貫で経験してきました。表面的な仕様をなぞるのではなく、その業務が「なぜそうなっているのか」まで踏み込んで理解することを大切にしています。
　最大の強みは、理解した業務を最後まで実装しきる完遂力です。商材出荷システムの開発では、「契約している限り請求が走る」システムと「請求の都度あらたに明細を発行する」という、思想の異なる 2 つの業務システムを 1 回の API で連携させる難しい要望がありました。両者の業務上の前提を読み解き、双方のライフサイクルを満たす中間データの持ち方を設計することで実現しています。また、Unity で作られたバーチャルフィッティングを WebGL 化して顧客の EC サイトへ埋め込む案件や、SOAP 連携しか用意されていない審査システムに REST 中間層を新設する案件など、前例や定石のない要求でも、技術と業務理解の両面から分解し、実装しきってきました。
　「つくる」だけでなく「動かしきる・成果を出しきる」ところまで責任を持ちます。対向システムが存在する開発では、自システムの作り込みより先に相手システムと疎通確認できる状態をつくり、インタフェース面の確認を徹底しました。その結果、運用保守ベンダーに 4 ヵ月と見積もられたシステムを 2 ヵ月で完成させ、当該システム起因の遅延・障害をゼロに抑え、社内表彰(社長賞)も複数回いただいています。
