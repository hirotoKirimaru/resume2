...
1on1支援SaaSの中核製品を**4.5年・755件のPull Request**にわたり開発してきたソフトウェアエンジニア。Ruby on Rails / Python(FastAPI) / TypeScript(Angular) / Terraform を横断するフルスタックで、機能開発からリアルタイム基盤の移行・可観測性・コスト最適化といった基盤健全化までを担う。

直近は新規AIプロダクトのPoCで、データ取り込み層・ナレッジグラフ（Temporal Knowledge Graph）・「業務の詰まり検出」の設計実装を主担当し、LLM × ナレッジグラフのアーキテクチャを技術選定から主導。並行して**673件のコードレビュー**を多言語・全社リポジトリ横断で担当し、チームの変更品質を担保してきた。


## 自己PR

1. **製品を長期間・横断的に支え続ける実装力と信頼性** — 4.5年・755PR・673レビューで中核製品を継続的に進化させてきた。
2. **0→1 を機能と運用基盤の両輪で立ち上げられる** — 新規プロダクトを機能だけでなく"回り続ける仕組み"まで構築した。
3. **事業のWhyから逆算してアーキテクチャを設計・技術選定できる** — AI/ナレッジグラフPoCを技術選定から主導。
4. **検証を"感覚"でなく"設計"でやる** — 評価駆動（混同行列・Precision/Recall）で精度を検証する。
5. **チーム横断の品質ゲートとしてのレビュー貢献** — 673件のコードレビューを多言語・全社横断で担当。

> 志向: 「製品を支える側から、製品の方向性を技術で決める側へ。特にLLM/データ基盤を、PoCで終わらせず事業価値が説明できる本番機能にすることに挑戦したい」

## 活かせる経験・スキル

- 大規模・長期運用SaaSのフルスタック開発（フロント / バックエンド / インフラ横断）
- エンタープライズ顧客の個別要件対応（SSO/SAML/2要素認証、データ統合、コンプライアンス設計）
...
フルスタックエンジニア（シニア / テックリード相当）として参画。4.5年・755件のPull Requestにわたり中核SaaSの継続開発、新規プロダクトの0→1立ち上げ、AI/LLMを活用したPoCの設計リード、673件のコードレビューを担当。
...
役割: ソフトウェアエンジニア（シニア / テックリード相当）。フルスタック実装・基盤移行・新規プロダクトのPoC設計リード・コードレビューを担当。

#### ① AIによるマネジメント支援プロダクトの PoC 設計リード

|期間|規模|役割|
|---|---|---|
|2024～現在|小規模|PoC設計リード|

- 技術スタック
    - Python, Anthropic API(Claude), Graphiti(Temporal KG), Neo4j / Amazon Neptune(openCypher), Kuzu, DSPy, Pydantic, PostgreSQL
- 担当業務

新規AIプロダクトのPoCで、業務データから「メンバーの詰まりどころ」を検出しマネジャーへ次の行動を提示する仕組みを、データ取り込み層・知識層・詰まり検出の3領域で設計・実装。LLM × Temporal Knowledge Graph のアーキテクチャを技術選定（ADR）から主導。

- 「詰まり検出」を *LLMへの丸投げ* から *型抽出済みグラフ構造でのカウント問題* へ捉え直し、retrieval / context organization の問題として再定義。
- バックエンド二系統戦略（ローカル=Neo4j / クラウド=Amazon Neptune）を openCypher 縛りで設計し、PoC段階からクラウド移植性を担保。
- ルール特徴量とグラフ由来factsを併用するハイブリッド詰まり判定を実装し、混同行列・Precision / Recall で汎化検証。
- DSPy による型付きパイプライン、spec-driven開発による設計の構造化。
...
- 技術スタック
    - Ruby on Rails, Centrifugo, Redis, Terraform, AWS(ECR/IAM)
- 担当業務

製品のリアルタイム通信を外部SaaS(PubNub)から自社運用のCentrifugo基盤へ移行。Terraform でECRリポジトリ・デプロイ用IAMロール・チャンネル名前空間・Redis(v6)設定を構築し、アプリ〜インフラを一気通貫で対応。"アプリも書けるSRE"としてアプリ・インフラ両面の実装を担当。

#### ③ 新規SaaSプロダクトの立ち上げ（0→1）

|期間|規模|役割|
|---|---|---|
|2024/02～現在|2名→1名|主力開発者|

- 技術スタック
    - Python(FastAPI), TypeScript, PostgreSQL, Sentry
- 担当業務

新規SaaSプロダクトをゼロベースから本番リリースまで構築（110 PR）。面談フロー・予約フロー・外部連携（カレンダー/データ連携）・メール配信・タイムゾーン統一などの機能をゼロから実装。Sentry / CI / リリースフロー / lint といった"回り続ける運用基盤"も整備。

- 2名（フロント1・バックエンド1）で立ち上げ、フロント担当退職後は1人で運用を継続。
- 導入先（人材サービス系エンタープライズ顧客）にてマネジャー約50名・メンバー約200名が利用。
...
|期間|規模|役割|
|---|---|---|
|2021/10～現在|〜25名|フルスタック開発者|

- 技術スタック
    - Ruby on Rails, Python(FastAPI), TypeScript/Angular, MySQL, GraphQL, Docker
- 担当業務

1on1支援SaaSの中核製品を、コア機能・エンタープライズ要件・データ整合性まで横断して継続開発。代表的な領域:

- **エンタープライズ認証基盤（23 PR / 2021〜）**: SSO/SAML設定の管理画面化、大手通信キャリア向け2要素認証(TOTP)、Google SSO(OAuth)からのカレンダー自動連携。IPアドレス制限などの個別要件対応。
- **マルチテナント統合**: 複数ワークスペースを1つに統合する要件に対し、既存データ（1on1記録・感想・ユーザー）を失わず・重複させず移行・集約。10ワークスペースをデータロスト0・無停止で統合完了。
- **集計グループ機能（55 PR / 3年）**: 組織構造に沿った集計のCSV一括設定、ペア単位集計、組織変更追従、削除データの整合性担保まで継続オーナーシップ。
- **メンター・メンティマッチング**: 自動マッチングの無限ループ回避、レコードロックに依存しない重複起動制御、削除時のデータ整合性堅牢化。リリース後、5社が導入。
- **定期フィードバック集計機能**: 機能を0→1で実装し、Lambdaで月次自動実行に乗せ、集計ロジックの不具合(割合100%超過)まで是正。
- カレンダー連携（Google/Outlook, 29 PR）、満足度・サーベイ集計、管理画面・CSV入出力など。

#### ⑤ 基盤健全化・コスト/性能最適化

|期間|規模|役割|
|---|---|---|
|2021/10～現在|単独〜小チーム|主担当|

- 技術スタック
    - Docker, uv, AWS(Graviton/ARM64), Datadog, GraphQL, GitHub Actions(CI)
- 担当業務

製品を止めずに進化させ続けるための、技術的負債・実行環境・可観測性のオーナーシップ。

- **CIパイプライン高速化**: テスト並列化を計測ベースで実験、差分なしファイルのスキップ、motoイメージのtmpfs化によるI/O削減。CI実行時間を30分→12分に短縮（60%削減）。
- **大規模基盤刷新（2026）**: Poetry→uv移行＋Dockerマルチステージビルド、全依存ライブラリの最新化（Python 3.12→3.13含む）、ARM64(Graviton)イメージ対応、Dockerイメージ圧縮のzstd化。
- **可観測性刷新**: Sentry → Datadog APM への監視統合、業務エラーの検知対象切り分け。
- **負債削減**: 未使用GraphQL操作30件と関連テストの削除、不要Redisキャッシュコードの削除。

#### ⑥ インフラ / IaC（兼務）

|期間|規模|役割|
|---|---|---|
|2021/10～現在|兼務|インフラ担当|

- 技術スタック
    - Terraform, AWS(ECS Fargate/ECR/IAM/Lambda/Redis), Datadog
- 担当業務

クラウドインフラをTerraformで管理（11 PR）。ECS Fargateのスペック最適化、Centrifugo基盤、Lambda管理、Datadog/Sentry連携。アプリ開発と兼務する形で"アプリも書けるDevOps"として関与。

#### ⑦ コードレビュー貢献

全期間で**673件**のコードレビューを、全社リポジトリ横断・多言語（Ruby/Python/TypeScript/HCL）で担当。ピーク年は303件。チームの変更品質ゲートとして機能。
...
|JavaScript|4年|
|Go|基礎|
|Visual Basic 6.0|1.5年|
|Visual Basic .NET|1.5年|
...
|技術|備考|
|---|---|
|Temporal KG設計|Graphiti/Neo4j/Neptune・openCypher|
|Anthropic API(Claude)|分類・抽出・パイプライン設計|
|DSPy|型付きLLMパイプライン|
|LLM評価設計|混同行列・Precision/Recall|

※PoC設計リードとして従事（本番運用は未了）

### インフラ / クラウド

|その他|年数|
|---|---|
|AWS(ECS Fargate/ECR/Lambda/IAM/Redis)|約3年|
|Terraform|約1.5年|
|Docker|約4年|
|Centrifugo|移行主担当|
|Datadog / Sentry|可観測性整備|
|GitHub Actions(CI)|CI設計・高速化|
|PostgreSQL|約4年|
|MySQL|約4年|
|Neo4j / Amazon Neptune|PoC期間|
|Oracle|3年|
|Ansible|1年|
|Azure|1年|
|Kubernetes(PKS)|0.5年|
