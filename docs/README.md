...
---
## 一言で表現する
製品を止めずに継続的に進化させ、答えのない新領域を設計判断でリードするエンジニア


## 職務要約

1on1 支援 SaaS の中核製品を**4.5 年・755 件の Pull Request**にわたり開発してきたソフトウェアエンジニア。Ruby on Rails / Python(FastAPI) / TypeScript(Angular) / Terraform を横断するフルスタックで、機能開発からリアルタイム基盤の移行・可観測性・コスト最適化といった基盤健全化までを担う。

直近は新規 AI プロダクトの PoC で、データ取り込み層・ナレッジグラフ（Temporal Knowledge Graph）・「業務の詰まり検出」の設計実装を主担当し、LLM × ナレッジグラフのアーキテクチャを技術選定から主導。並行して**673 件のコードレビュー**を多言語・全社リポジトリ横断で担当し、チームの変更品質を担保してきた。


## 自己PR

1. **製品を長期間・横断的に支え続ける実装力と信頼性** — 4.5 年・755PR・673 レビューで中核製品を継続的に進化させてきた
2. **0→1 を機能と運用基盤の両輪で立ち上げられる** — 新規プロダクトを機能だけでなく"回り続ける仕組み"まで構築した
3. **事業の Why から逆算してアーキテクチャを設計・技術選定できる** — AI/ナレッジグラフ PoC を技術選定から主導
4. **検証を"感覚"でなく"設計"でやる** — 評価駆動（混同行列・Precision/Recall）で精度を検証する
5. **チーム横断の品質ゲートとしてのレビュー貢献** — 673 件のコードレビューを多言語・全社横断で担当

> 志向: 「製品を支える側から、製品の方向性を技術で決める側へ。特にLLM/データ基盤を、PoCで終わらせず事業価値が説明できる本番機能にすることに挑戦したい」

## 活かせる経験・スキル

- 大規模・長期運用 SaaS のフルスタック開発（フロント / バックエンド / インフラ横断）
- エンタープライズ顧客の個別要件対応（SSO/SAML/2 要素認証、データ統合、コンプライアンス設計）
- リアルタイム通信基盤・依存基盤・実行環境の移行とコスト/性能最適化
- LLM × ナレッジグラフを用いたアプリケーション設計・技術選定（ADR）・評価設計
- 多言語横断のコードレビューによる品質ゲート
...
フルスタックエンジニア（シニア / テックリード相当）として参画。4.5 年・755 件の Pull Request にわたり中核 SaaS の継続開発、新規プロダクトの 0→1 立ち上げ、AI/LLM を活用した PoC の設計リード、673 件のコードレビューを担当。
...
#### （2）リアルタイム通信基盤の移行（PubNub → Centrifugo）
...
#### （4）中核製品の継続開発（フルスタック / 4.5年・598 PR）
